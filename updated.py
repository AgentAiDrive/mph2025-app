...
def load_json(path: str):
    """Load JSON data from a file or return a default."""
...
def render_step0():
    ...
    # CHATS
    with r1c2:
        titles = [f"{i+1}. {r['profile']} – {r['shortcut']}"
        ...
        render_home_card(
            ...
            buttons=[
                ...
                ("START CHAT","home_chat", None,
                    lambda: (st.session_state.update(step=7 if st.session_state.profiles else 1),
                             st.warning("No profiles – create one first.") if not st.session_state.profiles else None,
                             st.rerun())),
            ],
            expander_label="Chat History",
            expander_body=lambda: (
                [st.markdown(f"<p class='home-small'>{t}</p>", unsafe_allow_html=True) for t in titles]
    ...
    with r2c2:
        render_home_card(
            ...
            buttons=[("CLEAR DATA","home_clear",None,
                      lambda: (
                          st.session_state.update(profiles=[],
                                                   saved_responses=[]),
...
def render_step1():
    st.markdown('<div style="text-align:center;"><img src="https://img1.wsimg.com/isteam/ip/e13cd0a5-b867-446e-af2a-268488bd6f38/myparenthelpers%20logo%20round.png" width="80"/></div>', unsafe_allow_html=True)
    ...
    with c1:
        if st.button(" Parent", key="btn_parent"):
            st.session_state.agent_type="Parent"; st.session_state.step=2; st.rerun()
    with c2:
        if st.button(" Teacher", key="btn_teacher"):
            st.session_state.agent_type="Teacher"; st.session_state.step=2; st.rerun()
    with c3:
        if st.button("✨ Other", key="btn_other"):
            st.session_state.agent_type="Other"; st.session_state.step=2; st.rerun()
    render_top_nav()
...
def render_step2():
    ...
    with c1:
        if st.button("📚 Book", key="btn_book"):
            st.session_state.source_type="Book"; st.session_state.step=3; st.rerun()
    with c2:
        if st.button("🧠 Expert", key="btn_expert"):
            st.session_state.source_type="Expert"; st.session_state.step=3; st.rerun()
    with c3:
        if st.button("🎨 Style", key="btn_style"):
            st.session_state.source_type="Style"; st.session_state.step=3; st.rerun()
...
def render_step3():
    ...
    options = st.session_state['sources'][agent_type].get(st.session_state.source_type, [])
        ...
        with st.spinner("Thinking…"):
            try:
                prompt = (
                    ...
                    "In under 200 words, respond as JSON with 'persona_description'."
                )
                params = {"model":"gpt-4o","input":prompt,"response_format":{"type":"json_object"}}
                out = client.responses.create(**add_tool_params(params, {}))
                st.session_state.persona_description = json.loads(out.output_text)["persona_description"]
            except Exception as e:
...
def render_step6():
    ...
    <div class="home-card">
      ...
      <div class="answer-box">{profile['persona_description']}</div>
    </div>
    """, unsafe_allow_html=True)
    render_top_nav()
...
def render_step7():
    ...
    idx=st.selectbox("Agent:",range(len(names)),format_func=lambda i:names[i],key="chat_profile")
    sel=st.session_state.profiles[idx]

    # shortcut & memory toggle
    st.session_state.setdefault("shortcut"," DEFAULT")
    btn_cols=st.columns(len([" DEFAULT"," CONNECT"," GROW"," EXPLORE"," RESOLVE","❤ SUPPORT"]))
    shortcuts=[" DEFAULT"," CONNECT"," GROW"," EXPLORE"," RESOLVE","❤ SUPPORT"]
    for sc,col in zip(shortcuts,btn_cols):
        if st.button(sc.strip(),key=f"sc_{sc}",help=""):
            st.session_state.shortcut=sc
    st.checkbox("Use persistent memory",key="persistent_memory")

    # display history
    conv=st.session_state.conversation.get(sel['profile_name'],[]) if st.session_state.persistent_memory else st.session_state.temp_conversation.get(sel['profile_name'],[])
    for msg in conv:
        role="You" if msg['role']=="user" else "Agent"
        box_style="background:#144d2f;" if msg['role']=="user" else ""
        st.markdown(f"<div class='answer-box' style='{box_style}'><strong>{role}:</strong> {msg['content']}</div>",unsafe_allow_html=True)
    ...
    with c1:
        if st.button("Save Response",key="save_response"):
            record={"profile":sel['profile_name'],"shortcut":st.session_state.shortcut,"question":query,"answer":st.session_state.last_answer,"persistent_memory":st.session_state.persistent_memory}
    ...
    with c2:
        if st.button("SEND",key="send_btn"):
            base=(f"Adopt the persona: {sel['persona_description']}. "
            ...
            try:
                if st.session_state.persistent_memory:
                    conv_list=st.session_state.conversation.get(sel['profile_name'],[])
                    system_content=base+extras+"\nRespond as JSON with 'answer'."
                    messages=[{"role":"system","content":system_content}]+conv_list+[{"role":"user","content":query}]
                    params={"model":"gpt-4o","messages":messages,"response_format":{"type":"json_object"}}
                else:
                    params={"model":"gpt-4o","input":prompt,"response_format":{"type":"json_object"}}
                ...
                # update history
                if st.session_state.persistent_memory:
                    new_conv=conv_list+[{"role":"user","content":query},{"role":"assistant","content":answer}]
...
def render_step8():
    ...
    for field in ("profile","shortcut"):
        st.markdown(f"<p style='color:#fff;margin:4px 0;'><strong>{field.title()}:</strong> {item[field]}</p>",unsafe_allow_html=True)
    mem_flag=item.get("persistent_memory",False)
    st.markdown(f"<p style='color:#fff;margin:4px 0;'><strong>Persistent memory:</strong> {'Yes' if mem_flag else 'No'}</p>",unsafe_allow_html=True)
    st.markdown("<p style='color:#fff;margin:4px 0;'><strong>Question:</strong></p>",unsafe_allow_html=True)
    st.markdown(f"<blockquote style='color:#fff;border-left:4px solid #27e67a;padding-left:8px;margin:4px 0;'>{item['question']}</blockquote>",unsafe_allow_html=True)
    st.markdown("<p style='color:#fff;margin:4px 0;'><strong>Answer:</strong></p>",unsafe_allow_html=True)
    ...
    if item.get("conversation"):
        with st.expander("Show conversation history"):
            for msg in item["conversation"]:
                role="You" if msg["role"]=="user" else "Agent"
                st.markdown(f"<p style='color:#fff;margin:4px 0;'><strong>{role}:</strong> {msg['content']}</p>",unsafe_allow_html=True)
    c1,c2=st.columns(2)
    with c1:
...
def render_step10():
    ...
    items=sources[agent_type].get(source_type,[])
    st.write(f"**Current {source_type}s:**")
    ...
    with c1:
        if st.button("Remove Selected"):
            sources[agent_type][source_type]=[i for i in items if i not in to_remove]
            st.session_state['sources']=sources
            save_sources(sources); st.success("Removed!"); st.rerun()
    with c2:
        if st.button("Add"):
            if new_item and new_item not in items:
                sources[agent_type][source_type].append(new_item)
                st.session_state['sources']=sources
                save_sources(sources); st.success(f"Added '{new_item}'!"); st.rerun()
            elif new_item:
                st.warning("Already in list.")
    with c3:
        if st.button("Back to Home"):
            st.session_state.step=0; st.rerun()
    st.write(sources[agent_type][source_type])
    render_bottom_nav()
...
def main():
    ...
    {
        0: render_step0, 1: render_step1, 2: render_step2, 3: render_step3,
        4: render_step4, 5: render_step5, 6: render_step6, 7: render_step7,
        8: render_step8, 9: render_step9, 10: render_step10
